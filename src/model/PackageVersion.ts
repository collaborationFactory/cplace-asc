...
export class PackageVersion {
    ...
    public static initialize(mainRepo: string): PackageVersion {
        if (PackageVersion._version !== undefined) {
            throw new Error(
                `(PackageVersion) version has already been initialized`
            );
        }
        ...
        if (!fs.existsSync(packagePath)) {
            console.warn(
                cwarn`[NPM] Could not find package.json in repo ${mainRepo}...`
            );
            ...
            if (versionParts.length !== 3) {
                console.error(
                    cerr`[NPM] Expected package.json "version" to consist of 3 parts`
                );
                throw new Error(
                    `[NPM] Expected package.json "version" to consist of 3 parts`
                );
            }
    ...
    public static get(): PackageVersion {
        if (PackageVersion._version === undefined) {
            throw new Error(
                `(PackageVersion) version has not yet been initialized`
            );
        }
        return PackageVersion._version;
    }
