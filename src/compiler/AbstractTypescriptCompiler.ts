...
export abstract class AbstractTypescriptCompiler implements ICompiler {
    ...
    async compile(): Promise<CompilationResult> {
        ...
        console.log(`⟲ [${this.pluginName}] starting ${this.getJobName()} compilation...`);
        this.runTsc();
        let end = new Date().getTime();
        console.log(cgreen`⇢`, `[${this.pluginName}] ${this.getJobName()} compiled, starting bundling... (${formatDuration(end - start)})`);
        await this.doPostProcessing();
        end = new Date().getTime();
        console.log(GREEN_CHECK, `[${this.pluginName}] ${this.getJobName()} finished (${formatDuration(end - start)})`);

        try {
            const oldHash = this.readCompilationHash();
            const newHash = await this.computeAndUpdateCompilationHash();
            debug(`(TypescriptCompiler - ${this.getJobName()}) [${this.pluginName}] Old hash: ${oldHash} - New hash: ${newHash}`);
            ...
            debug(`(TypescriptCompiler - ${this.getJobName()}) [${this.pluginName}] Failed to get and compute hashes: ${e}`);
        }
    ...
    private getHashFilePath(): string {
        return path.resolve(this.assetsPath, this.outputDirName, AbstractTypescriptCompiler.HASH_FILE);
    }
    ...
    private computeAndUpdateCompilationHash(): Promise<string> {
        const generatedJsPath = path.resolve(this.assetsPath, this.outputDirName);
        const hashPath = this.getHashFilePath();
