...
const runCompilerCommand = (
    ...
    const { expectedMessages = [], expectedPatterns = [], shouldSucceed = true } = expectations;
...
const runWatchModeTest = async (expectedMessage: string, timeoutMs: number = 10000): Promise<void> => {
        ...
        const output = await new Promise<string>((resolve, reject) => {
            ...
            const finish = (result: string, isTimeout: boolean = false) => {
                ...
                if (isTimeout && !result.includes(expectedMessage)) {
                    reject(new Error(`Timeout: Expected message "${expectedMessage}" not found in output: ${result}`));
                    return;
                }
...
describe('Assets Compiler E2E Tests', () => {
    ...
    describe('TC8: Verify compilation completed message for watch mode', () => {
        it('should display compilation started message in watch mode', async () => {
    ...
    describe('TC10: Verify compilation completed message after version check', () => {
        it('should display compilation completed message after version check', () => {
            verifyCompilationSuccess(['-v']);
        }, timeout);
    });

    describe('TC11: Verify assets compiler with -o flag (only preprocessing)', () => {
        it('should verify assets compiler with -o flag for only preprocessing', () => {
            verifyBasicCompilerOutput(['-o']);
        }, timeout);
    });

    describe('TC12: Verify preprocessing completed message', () => {
        it('should display preprocessing completed message', () => {
    ...
    describe('TC14: Verify assets compiled successfully message', () => {
        it('should display assets compiled successfully message', () => {
            verifyCompilationSuccess(['-P']);
        }, timeout);
    });

    describe('TC19: Verify flag conflict error handling', () => {
        it('should handle conflicting flags appropriately', () => {
            runCompilerCommand(['--watch', '--onlypre'], {
                expectedMessages: ['--watch and --onlypre cannot be enabled simultaneously'],
                shouldSucceed: false
            });
        }, timeout);
    });
    ...
    describe('TC20: Verify undefined properties error handling', () => {
        it('should handle undefined properties error', () => {
