...
describe('CplaceTSConfigGenerator', () => {
    ...
    beforeEach(() => {
        ...
        mockPlugin = {
            ...
            getPluginPathRelativeFromRepo: jest.fn().mockImplementation((sourceRepo: string, localOnly: boolean) => {
        ...
        (AssetsCompiler.isLocalParentRepo as jest.Mock).mockImplementation((repoName: string) => true);
        (AssetsCompiler.isArtifactsOnlyBuild as jest.Mock).mockReturnValue(false);

    });

    describe('getRelativePathToMain', () => {
        it('should return correct path when localOnly is true', () => {
            const generator = new CplaceTSConfigGenerator(mockPlugin, mockDependencies, true, false);
            const result = generator.getRelativePathToMain(true, 'test-repo', '../../..');
            expect(result).toBe(path.join('../../..'));
        });

        it('should return path to main folder when repo is different from main', () => {
            const generator = new CplaceTSConfigGenerator(mockPlugin, mockDependencies, false, false);
        ...
        it('should return current repo in artifacts-only build case', () => {
            ...
            (AssetsCompiler.isArtifactsOnlyBuild as jest.Mock).mockReturnValue(true);
    ...
    describe('getRelativePathToPlatform', () => {
        it('should return correct platform path for local plugin', () => {
            ...
            expect(result).toBe(path.join('../../../../main/cf.cplace.platform'));
        });

        it('should throw error when platform plugin not found', () => {
            expect(() => new CplaceTSConfigGenerator(mockPlugin, [], false, false)).toThrow('Platform plugin not found');
        });
        ...
        it('should return path in node_modules when platform is an artifact plugin', () => {
            ...
            const generator = new CplaceTSConfigGenerator(mockPlugin, [artifactPlatform], false, false);
            const result = generator.getRelativePathToPlatform();
            expect(result).toBe(path.join('../../../node_modules/@cplace-assets/cplace_cf-cplace-platform'));
        });
    });
    ...
    describe('getPathsAndRefs', () => {
        it('should return correct paths and refs for local plugins', () => {
            ...
            expect(Object.keys(result.paths)).toContain('@cf.cplace.platform/*');
        ...
        it('should handle multiple dependencies correctly', () => {
            const mockDep = {
                ...
                getPluginPathRelativeFromRepo: jest.fn().mockReturnValue('test.dependency')
            } as unknown as CplacePlugin;

            const generator = new CplaceTSConfigGenerator(mockPlugin, [...mockDependencies, mockDep], false, false);
            const result = generator.getPathsAndRefs();
    ...
    describe('getTsConfigBasePath', () => {
        it('should return node_modules path when platform is artifacts plugin', () => {
            ...
            expect(result).toBe(path.join('../../../node_modules/@cplace-assets/cplace_cf-cplace-platform/tsconfig.base.json'));
        });
        ...
        it('should return path from platform for artifacts build and local main repo', () => {
            ...
            expect(result).toContain(path.join('../../../../main/cf.cplace.platform/assets/tsconfig.base.json'));
        });
        ...
        it('should return path from main repo for normal build', () => {
            ...
            expect(result).toContain(path.join('../../../../main/tsconfig.base.json'));
        });
    });
});