...
describe('Assets Compiler E2E Tests', () => {
  ...
  const getCommand = (flag: string = '') => {
    const script = isWindows ? './assets-compiler.cmd' : './assets-compiler.sh';
  ...
  afterEach(() => {
    ...
    try {
      if (!isWindows) {
        execSync('pkill -f "assets-compiler" || true', { stdio: 'ignore' });
      }
    } catch (error) {
  ...
  describe('TC1: Basic Assets Compiler Verification', () => {
    it('should verify assets compiler with correct Node version', () => {
      ...
      try {
        ...
        expect(result).toContain('You are using a correct Node version');
